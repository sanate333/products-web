...
export default function WhatsAppBot() {
  ...
  const openChat = useCallback(async (chat) => {
    ...
    // Refrescar foto de perfil si no la tiene
    if (!chat.photoUrl) {
      try {
        const r = await fetch(`${API}/chats/${encodeURIComponent(chat.chatId)}/photo`);
        const d = await r.json();
        if (d.ok && d.photoUrl) {
          setActiveChat((prev) => prev ? { ...prev, photoUrl: d.photoUrl } : prev);
          setChats((prev) => prev.map((c) => c.chatId === chat.chatId ? { ...c, photoUrl: d.photoUrl } : c));
        }
      } catch {}
    }
  }, [loadMessages]);
  ...
  const pollWaStatus = useCallback(async () => {
    try {
      ...
      setWaPhone(d.phone || '');
      if (d.status === 'connecting' || d.status === 'disconnected') {
  ...
  const disconnectWa = useCallback(async () => {
    try {
      ...
      const d = await r.json();
      if (!d.ok) throw new Error(d.error || 'No se pudo desconectar');
  ...
  useEffect(() => {
    ...
    loadTriggers();
    pollWaStatus();
  }, [loadChats, loadTemplates, loadTriggers, pollWaStatus]);
  ...
  }, [activeChat?.chatId, loadChats, loadMessages, pollWaStatus]);
