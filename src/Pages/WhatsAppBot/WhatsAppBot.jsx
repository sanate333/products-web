...
export default function WhatsAppBot() {
          ...
          try {
            ...
            if (d.type === 'chat_update') {
              setChats(prev => {
                ...
                if (idx === -1) { loadC().then(function() { loadIGChats(); }).catch(() => {}); return prev }
                const next = [...prev]
              ...
              setStatus(s)
              if (s === 'connected') loadC().then(function() { loadIGChats(); }).catch(() => {})
            }
          } catch {}
        }
  ...
  async function ping() {
    try {
      ...
      setPhone(d.phone || '')
      if (s === 'connected') {
        try { await loadC().then(function() { loadIGChats(); }) } catch {}
    ...
    if (id === 'conexion') {
      if (status === 'connected') { setTimeout(drawQRConnected, 120) }
        ...
        setTimeout(async () => {
          ...
          else if (s === 'connected') { loadC().then(function() { loadIGChats(); }).catch(() => {}); setTimeout(drawQRConnected, 150) }
          else { regenerateQR() }
        }, 100)
      }
    }
            ...
            <div className="wbv5-chat-wrap">
              <div className="wbv5-inbox-list">
                <div className="wbv5-il-header">
                  ...
                  <button className="wbv5-btn wbv5-btn-green wbv5-btn-sm" onClick={() => loadC().then(function() { loadIGChats(); }).catch(() => {})}>ð</button>
                </div>
