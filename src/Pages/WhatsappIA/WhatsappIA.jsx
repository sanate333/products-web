...
const WA_API = 'http://localhost:5055/api/whatsapp';
...
export function WhatsappConfig() {
    ...
    const pollStatus = useCallback(async () => {
        try {
            ...
            const s = String(data.status || 'disconnected').toLowerCase();
            setConnectionState(['connected', 'disconnected', 'connecting'].includes(s) ? s : 'disconnected');
            setPhone(data.phone || '');
            if (s !== 'connected') {
                const qRes = await fetch(`${WA_API}/qr`);
                const qData = await qRes.json();
                setQrDataUrl(qData.qr || null);
            } else {
                setQrDataUrl(null);
            }
            setNotice('');
            ...
            setNotice('Backend no disponible en localhost:5055 — ejecuta npm start');
        }
    }, []);

    useEffect(() => {
        pollStatus();
        const iv = setInterval(pollStatus, 6000);
        return () => clearInterval(iv);
    }, [pollStatus]);

    const reconnect = async () => {
        setConnectionState('connecting');
        setNotice('Iniciando conexion WhatsApp...');
        try {
            ...
            <section className="botCard">
                ...
                <div className="botConfigGrid">
                    ...
                    <div className="configContent">
                        {!backendOnline ? (
                            <article className="connectionCard" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 10 }}>
                                <h4>⚠️ Servidor WhatsApp no disponible</h4>
                                <p style={{ margin: '4px 0', color: '#555' }}>El servidor Node.js no está corriendo en <strong>localhost:5055</strong>.</p>
                                <p style={{ margin: '4px 0', color: '#555' }}>Abre una terminal en el proyecto y ejecuta:</p>
                                <pre style={{ background: '#1e1e2e', color: '#a6e3a1', padding: '8px 14px', borderRadius: 6, fontFamily: 'monospace', margin: '6px 0' }}>npm start</pre>
                                <p style={{ margin: '4px 0', color: '#555' }}>Luego recarga esta página.</p>
                            </article>
                        ) : (
                            <article className="connectionCard">
                                <div>
                                    ...
                                    {phone ? <small className="waConnectionNotice">📱 {phone}</small> : null}
                                    {notice ? <small className="waConnectionNotice">{notice}</small> : null}
                                </div>
                                <div className="connectionActions">
                                    {connectionState !== 'connected' && (
                                        <button type="button" className="connectQrBtn" onClick={() => { setShowQrModal(true); pollStatus(); }}>
                                            Conectar con QR
                                        </button>
                                    )}
                                    {connectionState !== 'connected' && (
                                    ...
                                    {connectionState === 'connected' && (
                                        <button type="button" onClick={disconnect} style={{ background: '#e53e3e', color: '#fff', border: 'none', borderRadius: 6, padding: '6px 14px', cursor: 'pointer' }}>
                                            Desconectar
                                        </button>
                                    )}
                                </div>
                ...
                <div className="botModalOverlay" onClick={() => setShowQrModal(false)}>
                    <div className="botQrModal" onClick={(event) => event.stopPropagation()}>
                        <div className="botQrModalHead">
                            ...
                            <button type="button" onClick={() => setShowQrModal(false)}>✕</button>
                        </div>
                        {qrDataUrl ? (
                            <img src={qrDataUrl} alt="QR WhatsApp" style={{ width: 220, height: 220, borderRadius: 8, border: '2px solid #e2e8f0', display: 'block', margin: '0 auto' }} />
                        ) : connectionState === 'connected' ? (
                            <div className="qrBox" style={{ color: '#16a34a', fontSize: 14 }}>✅ Ya conectado: {phone}</div>
                        ) : (
                            <div className="qrBox" style={{ color: '#888', fontSize: 13 }}>Generando QR... espera unos segundos.</div>
                        )}
