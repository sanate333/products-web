...
const PLATFORM_RULES = {
  facebook_marketplace: { label: "Facebook Marketplace", maxDaily: 8, minSpacingMin: 90, emoji: "\uD83C\uDFEA" },
  instagram_feed: { label: "Instagram Feed", maxDaily: 5, minSpacingMin: 120, emoji: "\uD83D\uDCF8" },
  instagram_story: { label: "Instagram Story", maxDaily: 8, minSpacingMin: 60, emoji: "\uD83D\uDCF1" },
  facebook_page: { label: "Facebook Page", maxDaily: 5, minSpacingMin: 120, emoji: "\uD83D\uDCD8" }
};
...
export default function MarketingRedes() {
  ...
  const handleConnect = async (platform) => {
    setLoading(true);
    showMsg("Iniciando conexion con " + platform + "...");
    try {
      ...
      if (result.oauth_url) {
        ...
        const handler = (event) => {
          if (event.data?.type === "ig_connected") {
            ...
            showMsg("Conectado exitosamente como @" + (event.data.data?.username || "cuenta") + "!", "success");
            loadConnections();
          } else if (event.data?.type === "ig_error") {
            window.removeEventListener("message", handler);
            showMsg("Error en conexion: " + event.data.error, "error");
          }
        };
      ...
      showMsg("Error de conexion: " + err.message, "error");
    }
    setLoading(false);
  };
  ...
  const handlePublish = async (post) => {
    ...
    for (const plat of platforms) {
      try {
        ...
        if (plat === "instagram_feed") {
          ...
          payload = { message: post.title + "\n\n" + post.description + (post.price ? "\n$ " + post.price : ""), image_url: post.images || undefined, store_id: "default" };
        }
    ...
    if (success.length > 0) showMsg("Publicado en " + success.length + " plataforma(s)" + (failed.length > 0 ? ", " + failed.length + " fallaron" : ""), "success");
    else showMsg("Error al publicar: " + failed.map(f => f.message || f.error).join(", "), "error");
  ...
  const handleSchedulePost = async (e) => {
    e.preventDefault();
    if (!newPost.title) return showMsg("Titulo requerido", "error");
    ...
    try {
      ...
      if (result.error || (Array.isArray(result) && result[0]?.error)) {
        ...
        if (!newPost.scheduled_at) {
          ...
          showMsg("Publicacion programada exitosamente", "success");
        }
  ...
  const cardStyle = { background: "#1e1e2e", borderRadius: 12, padding: 20, marginBottom: 16 };
  const rowStyle = { display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 };
  const btnBase = { padding: "8px 16px", borderRadius: 8, border: "none", cursor: "pointer", fontWeight: 600 };
  const inputStyle = { width: "100%", padding: "10px 14px", borderRadius: 8, border: "1px solid #444", background: "#1a1a2e", color: "#fff", fontSize: "0.95rem" };
  const labelStyle = { fontWeight: 600, display: "block", marginBottom: 6 };
  const smallText = { fontSize: "0.85rem", color: "#888" };
    ...
    <div className="containerGrid">
      ...
      <section className="containerSection">
        <div className="marketing-redes">
          ...
          <h2 style={{ marginBottom: 8, fontSize: "1.5rem" }}>Marketing Redes</h2>
          <p style={{ color: "#888", marginBottom: 20, fontSize: "0.9rem" }}>Conecta tus redes sociales y autopublica productos en multiples plataformas.</p>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
            {[
              ...
              { id: "autopublicar", label: "Autopublicador" },
              { id: "historial", label: "Historial" },
              { id: "reglas", label: "Reglas Anti-Ban" }
            ].map(t => (
          ...
          {tab === "conexiones" && (
            <div>
              ...
              <div style={cardStyle}>
                <div style={rowStyle}>
                  <div>
                    <span style={{ fontSize: "1.1rem", fontWeight: 600 }}>Instagram</span>
                    {getConnectionStatus("instagram") ? (
                      <span style={{ marginLeft: 12, color: "#10b981", fontWeight: 600 }}>
                        Conectado como @{getConnectionStatus("instagram").ig_username || "cuenta"}
                      </span>
                    ) : (
                      <span style={{ marginLeft: 12, color: "#f59e0b" }}>No conectado</span>
                    )}
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    {getConnectionStatus("instagram") ? (
                      ...
                      <button onClick={() => handleConnect("instagram")} disabled={loading}
                        style={{ ...btnBase, background: "#e1306c", color: "#fff" }}>
                ...
                {getConnectionStatus("instagram") && (
                  <div style={{ marginTop: 12, ...smallText }}>
                    <p>Token tipo: {getConnectionStatus("instagram").token_type || "N/A"} | Page Token: {getConnectionStatus("instagram").page_token ? "Si" : "No"} | Pagina: {getConnectionStatus("instagram").page_name || "N/A"}</p>
                    ...
                    <p style={{ color: "#10b981", marginTop: 4 }}>Puede publicar en: Instagram Feed, Instagram Stories{getConnectionStatus("instagram").page_token ? ", Facebook Page" : ""}</p>
                  </div>
                )}
              </div>
              ...
              <div style={cardStyle}>
                <div style={rowStyle}>
                  <div>
                    <span style={{ fontSize: "1.1rem", fontWeight: 600 }}>Facebook Page</span>
                    {getConnectionStatus("facebook_page") ? (
                      <span style={{ marginLeft: 12, color: "#10b981", fontWeight: 600 }}>
                        Conectado - {getConnectionStatus("facebook_page").page_name || "Pagina"}
                      </span>
                    ) : (
                      <span style={{ marginLeft: 12, color: "#f59e0b" }}>{connections.length > 0 ? "Conecta Instagram con una Pagina de Facebook vinculada" : "No conectado"}</span>
                    )}
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    {!getConnectionStatus("facebook_page") && (
                ...
                {getConnectionStatus("facebook_page") && (
                  <div style={{ marginTop: 12, ...smallText }}>
                    <p>Pagina ID: {getConnectionStatus("facebook_page").page_id} | Nombre: {getConnectionStatus("facebook_page").page_name}</p>
                    <p style={{ color: "#10b981" }}>Puede publicar en: Facebook Page (fotos, textos, links)</p>
                  </div>
                )}
              </div>
              ...
              <div style={cardStyle}>
                <div style={rowStyle}>
                  <div>
                    <span style={{ fontSize: "1.1rem", fontWeight: 600 }}>Facebook Marketplace</span>
                    {getConnectionStatus("facebook_marketplace") ? (
                      <span style={{ marginLeft: 12, color: "#10b981", fontWeight: 600 }}>Disponible via Facebook Page</span>
                    ) : (
                      <span style={{ marginLeft: 12, color: "#f59e0b" }}>Requiere conexion de Facebook Page</span>
                    )}
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    {!getConnectionStatus("facebook_marketplace") && (
                ...
                <div style={{ marginTop: 12, ...smallText }}>
                  <p>Marketplace usa la conexion de Facebook Page para publicar productos. Las publicaciones aparecen en tu pagina y pueden llegar a Marketplace.</p>
                  {getConnectionStatus("facebook_marketplace") && (
                    <p style={{ color: "#10b981", marginTop: 4 }}>Las publicaciones en Facebook Page pueden aparecer en Marketplace automaticamente</p>
                  )}
                </div>
              </div>
              ...
              <div style={{ background: "#1a1a2e", borderRadius: 12, padding: 16, border: "1px solid #333", marginTop: 16 }}>
                <p style={{ fontWeight: 600, marginBottom: 8 }}>Informacion de Conexion</p>
                <p style={{ ...smallText, lineHeight: 1.6 }}>
                  Al conectar Instagram con Facebook Login, se obtiene acceso a: Instagram Feed, Instagram Stories y Facebook Page (si la cuenta de Instagram esta vinculada a una pagina).
                  Para Marketplace, las publicaciones se realizan a traves de la pagina de Facebook y pueden aparecer automaticamente en Marketplace segun la configuracion de Facebook.
                </p>
              </div>
            </div>
          )}
          ...
          {tab === "autopublicar" && (
            <div>
              <h3 style={{ marginBottom: 16 }}>Crear Publicacion</h3>

              <form onSubmit={handleSchedulePost} style={cardStyle}>

                {/* Platform selector */}
                <div style={{ marginBottom: 20 }}>
                  <label style={labelStyle}>Plataformas de destino:</label>
                    ...
                    {Object.entries(PLATFORM_RULES).map(([key, rule]) => {
                      ...
                      return (
                        <button type="button" key={key}
                          ...
                          {isSelected && isConnected && <span style={{ display: "block", fontSize: "0.7rem", color: "#10b981" }}>Seleccionado</span>}
                        </button>
                      );
                    })}
                  </div>
                ...
                {newPost.platforms.includes("facebook_marketplace") && (
                  <div style={{ marginBottom: 20, background: "#2a2a3e", borderRadius: 10, padding: 14, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div>
                      <span style={{ fontWeight: 600 }}>Mostrar a amigos</span>
                      <p style={{ fontSize: "0.8rem", color: "#888", marginTop: 4 }}>Cuando esta activo, tus amigos de Facebook pueden ver tus publicaciones de Marketplace.</p>
                    </div>
                    <button type="button"
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <input value={newPost.title} onChange={e => setNewPost({ ...newPost, title: e.target.value })}
                    placeholder="Nombre del producto o publicacion"
                    style={inputStyle} />
                </div>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <textarea value={newPost.description} onChange={e => setNewPost({ ...newPost, description: e.target.value })}
                    rows={4} placeholder="Descripcion del producto o publicacion..."
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <p style={{ fontSize: "0.75rem", color: "#888", marginTop: 4 }}>
                    La imagen debe ser una URL publica accesible. Instagram requiere imagenes JPEG de minimo 150x150px.
                  </p>
                </div>
                ...
                {/* Schedule */}
                <div style={{ marginBottom: 20 }}>
                  <label style={labelStyle}>Programar publicacion (opcional)</label>
                ...
                <div style={{ display: "flex", gap: 12 }}>
                  <button type="submit" disabled={loading}
                    ...
                    {loading ? "Procesando..." : newPost.scheduled_at ? "Programar" : "Publicar Ahora"}
                  </button>
                </div>
              </form>
              ...
              {posts.filter(p => p.status === "scheduled" || p.status === "draft").length > 0 && (
                <div style={{ marginTop: 24 }}>
                  ...
                  {posts.filter(p => p.status === "scheduled" || p.status === "draft").map(post => (
                    <div key={post.id} style={{ ...cardStyle, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
                      <div>
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                ...
                <button onClick={() => { loadLogs(); loadPosts(); }}
                  style={{ ...btnBase, background: "#2a2a3e", color: "#aaa" }}>
                  Actualizar
                </button>
              </div>
              ...
              {posts.filter(p => p.status === "published").length > 0 && (
                <div style={{ marginBottom: 24 }}>
                  <h4 style={{ color: "#10b981", marginBottom: 10 }}>Publicados</h4>
                  {posts.filter(p => p.status === "published").map(post => (
                    <div key={post.id} style={{ ...cardStyle, padding: 14 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap" }}>
                        <strong>{post.title}</strong>
                        <span style={smallText}>{post.published_at ? new Date(post.published_at).toLocaleString() : ""}</span>
                      </div>
                      <p style={{ ...smallText, marginTop: 4 }}>Plataformas: {post.platform}</p>
                    </div>
                     ))}
                </div>
              )}
              ...
              {/* Publish log */}
              <h4 style={{ marginBottom: 10 }}>Log de Publicaciones</h4>
              {logs.length === 0 ? (
                <p style={{ color: "#888" }}>No hay registros aun.</p>
              ) : (
                logs.map(log => (
                  <div key={log.id} style={{ ...cardStyle, padding: 14 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap" }}>
                      <span>
                        {log.status === "success" ? "[OK]" : "[ERROR]"} <strong>{log.post_title}</strong>
                      </span>
                      <span style={smallText}>{log.published_at ? new Date(log.published_at).toLocaleString() : ""}</span>
                    </div>
              ...
              {posts.length === 0 && logs.length === 0 && (
                <div style={{ textAlign: "center", padding: 40, color: "#888" }}>
                  <p style={{ fontSize: "1.5rem" }}>Sin publicaciones</p>
                  <p>No hay publicaciones ni registros aun. Crea tu primera publicacion en el Autopublicador.</p>
                </div>
              )}
            </div>
          )}
          ...
          {tab === "reglas" && (
            <div>
              <h3 style={{ marginBottom: 16 }}>Reglas Anti-Ban por Plataforma</h3>
              <p style={{ color: "#888", marginBottom: 20, fontSize: "0.9rem" }}>
                Estas reglas protegen tus cuentas de bloqueos por publicacion excesiva. Los limites estan configurados de forma conservadora.
              </p>

              {Object.entries(PLATFORM_RULES).map(([key, rule]) => (
                <div key={key} style={cardStyle}>
                  <h4 style={{ marginBottom: 10 }}>{rule.emoji} {rule.label}</h4>
                  <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
                    <div style={{ background: "#2a2a3e", borderRadius: 8, padding: "10px 16px", flex: 1, minWidth: 150 }}>
                      <p style={smallText}>Maximo diario</p>
                      <p style={{ fontSize: "1.3rem", fontWeight: 700 }}>{rule.maxDaily} <span style={{ fontSize: "0.8rem", fontWeight: 400 }}>publicaciones</span></p>
                    </div>
                    <div style={{ background: "#2a2a3e", borderRadius: 8, padding: "10px 16px", flex: 1, minWidth: 150 }}>
                      <p style={smallText}>Espaciado minimo</p>
              ...
              <div style={{ background: "#1a1a2e", borderRadius: 12, padding: 16, border: "1px solid #333", marginTop: 16 }}>
                <p style={{ fontWeight: 600, marginBottom: 8 }}>Consejos Anti-Ban</p>
                <ul style={{ color: "#888", fontSize: "0.85rem", lineHeight: 1.8, paddingLeft: 20 }}>
                  <li>No publiques el mismo contenido en multiples plataformas simultaneamente</li>
                  <li>Varia los textos y descripciones entre publicaciones</li>
                  <li>Respeta los intervalos minimos entre publicaciones</li>
