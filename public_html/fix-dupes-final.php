...
foreach ($fixes as $fix) {
    $id = $fix[0];
    $img = $fix[1];
    $stmt = $pdo->prepare("UPDATE productos SET imagen1 = ? WHERE idProducto = ?");
    $stmt->execute(array($img, $id));
    echo "ID $id => $img (rows: " . $stmt->rowCount() . ")\n";
}

echo "\n--- DUPLICATES CHECK ---\n";
$stmt = $pdo->query("SELECT imagen1, COUNT(*) as c, GROUP_CONCAT(idProducto) as ids FROM productos GROUP BY imagen1 HAVING c > 1");
$dupes = $stmt->fetchAll(PDO::FETCH_ASSOC);
if (empty($dupes)) {
    echo "ZERO duplicates! All products unique.\n";
} else {
    foreach ($dupes as $d) {
